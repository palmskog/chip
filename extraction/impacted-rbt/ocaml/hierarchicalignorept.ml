...
let new_top_vertex_entry json =
  let id = to_int (member "id" json) in
  let uri = to_string (member "uri" json) in
  let checkable = to_bool (member "checkable" json) in
  let contents = List.map to_int (to_list (member "contents" json)) in
  let checksum = to_string (member "checksum" json) in
  (id, uri, checkable, contents, checksum)

let new_bot_vertex_entry json =
  let id = to_int (member "id" json) in
  let uri = to_string (member "uri" json) in
  let checkable = to_bool (member "checkable" json) in
  let checksum = to_string (member "checksum" json) in
...
let build_old_bot_idx_arr old_id_idx_tbl el =
  ...
  List.iter
    (fun (id, uri, checkable, neighbors, checksum) ->
      ...
      let al = List.fold_left (fun l aid ->
        match Hashtbl.find_opt old_id_idx_tbl aid with
        | None -> l
        | Some iid -> iid :: l) [] neighbors
      in
...
let build_old_top_idx_arr old_top_id_idx_tbl old_bot_id_idx_tbl el =
  ...
  List.iter
    (fun (id, uri, checkable, neighbors, contents, checksum) ->
      ...
      let al = List.fold_left (fun l aid ->
        match Hashtbl.find_opt old_top_id_idx_tbl aid with
        | None -> l
        | Some iid -> iid :: l) [] neighbors
      in
      let cl = List.fold_left (fun l aid ->
        match Hashtbl.find_opt old_bot_id_idx_tbl aid with
        | None -> l
        | Some iid -> iid :: l) [] contents
      in
...
let extend_bot_id_idx_tbl tbl el =
...
let extend_top_id_idx_tbl tbl el =
  let idx = ref (Hashtbl.length tbl) in
  List.iter
    (fun (id, _,_, _, _) ->
      if not (Hashtbl.mem tbl id) then
...
let build_top_new_idx_arr new_top_id_idx_tbl new_bot_id_idx_tbl el =
  let arr = Array.make (List.length el) (0, "", false, [], "") in
  List.iter
    (fun (id, uri, checkable, contents, checksum) ->
      let idx = Hashtbl.find new_top_id_idx_tbl id in
      let cl = List.fold_left (fun l aid ->
        match Hashtbl.find_opt new_bot_id_idx_tbl aid with
        | None -> l
        | Some iid -> iid :: l) [] contents
      in      
      arr.(idx) <- (id, uri, checkable, cl, checksum))
    el;
  arr

let build_bot_new_idx_arr new_id_idx_tbl el =
  let arr = Array.make (List.length el) (0, "", false, "") in
  List.iter
...
let () =
  ...
  let new_bot_entries = List.map new_bot_vertex_entry bot_new in
  let new_top_entries = List.map new_top_vertex_entry top_new in
  ...
  let new_bot_id_idx_tbl = extend_bot_id_idx_tbl old_bot_id_idx_tbl new_bot_entries in
  let new_bot_idx_arr = build_bot_new_idx_arr new_bot_id_idx_tbl new_bot_entries in

  let new_top_id_idx_tbl = extend_top_id_idx_tbl old_top_id_idx_tbl new_top_entries in
  let new_top_idx_arr = build_top_new_idx_arr new_top_id_idx_tbl new_bot_id_idx_tbl new_top_entries in 
  ...
  let top_f_new k = let (_,_,_,_,checksum) = new_top_idx_arr.(k) in checksum in
  ...
  let partition' k = let (_,_,_,contents,_) = new_top_idx_arr.(k) in contents in

  let bot_chk_imp_fr =
    Change_impact.hierarchical_checkable_impacted_fresh_pt
