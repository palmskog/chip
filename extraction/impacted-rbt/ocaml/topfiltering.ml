...
let build_new_idx_arr new_id_idx_tbl el =
  ...
  List.iter
    (fun (id, uri, adjacent, runnable, checksum) ->
      ...
      let al = List.map (Hashtbl.find new_id_idx_tbl) adjacent in
      arr.(idx) <- (id, uri, al, runnable, checksum))
    el;
  arr
...
let build_succs'_arr new_idx_arr =
  let arr = Array.make (Array.length new_idx_arr) [] in
  Array.iteri
    (fun idx (_, _, adjacent, _, _) ->
      List.iter (fun idx' -> arr.(idx') <- idx :: arr.(idx')) adjacent)
    new_idx_arr;
  arr
...
let () =
  ...
  let succs'_arr = build_succs'_arr new_idx_arr in
  ...
  let chk k = let (_,_,_,runnable,_) = new_idx_arr.(k) in runnable in
  let successors' k = succs'_arr.(k) in

  let tsorted =
    Change_impact.topsort
      num_new num_old
      successors
      f_new f_old
      chk
  ...
  let res =
    List.map
      (fun k -> let (_,uri,_,_,_) = new_idx_arr.(k) in uri)
      tsorted
  in
